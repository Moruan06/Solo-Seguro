...
Monitoramento inteligente de solo e ambiente com sensores físicos, IoT e visualização em tempo real.
...
- **Captação de dados com sensores**: umidade do solo, temperatura/umidade do ar, vibração/inclinação e chuva (conjunto flexível e expansível).
...
Sensores → Gateway IoT → Backend/Cloud → API/WS → Frontend Solo-Seguro
...
- **Backend/Cloud**: persiste dados, aplica regras de alerta e expõe APIs/WebSocket.
- **Frontend**: consome as APIs, exibe dashboards, histórico e status dos dispositivos.
...
- Mapa com camadas de risco e pontos de leitura
